...
class OkeWorkloadIdentityProvider
    ...
    private ReactorHttpClient okeTokenClient;
    ...
    private String getSecurityToken() {
        ...
        HttpResponse response = okeTokenClient.postRequest(
            tokenURL.toString(), headers(saToken, requestId),
            Unpooled.wrappedBuffer(payloadByte) /*,timeoutMs, logger*/ ).block();

        final int responseCode = response.getStatusCode();
        String responseOutput = response.getBodyAsStringSync();
        if (responseCode > 299) {
